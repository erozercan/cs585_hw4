...
class CamVidDataset(Dataset):
    ...
    def parse_class_dict(self, class_dict_path):
        ...
        dicc = pd.read_csv(class_dict_path)

        dictt=dict()

        for x in range(32):
            val=dicc.iloc[x]
            dictt[x]=((val[1],val[2],val[3]), val[0])
    ...
    def rgb_to_class_id(self, label_img):
        ...
        dictt_vals=[i[0] for i in list(self.class_dict.values())]
        ...
        for y in range(height):
            ...
            for x in range(width):
                ...
                rgb= label_img.getpixel((x, y))

                internal +=[dictt_vals.index(rgb)]
        
            id_img += [internal]


        image_array = np.array(id_img, dtype=np.uint8)
